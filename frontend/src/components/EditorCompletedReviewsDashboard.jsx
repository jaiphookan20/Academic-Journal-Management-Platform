...
const EditorCompletedReviewsDashboard = () => {
    ...
    const tableStyle = {
        ...
        const fetchData = async () => {
          try {
            const response = await fetch(
              `http://${process.env.REACT_APP_API_HOST}:8080/editorial-decisions/get-all-completed-submissions`,{
                method: "GET",
                headers: {
      ...
      const authorNamesArray = completedSubmissions.map(item => extractAuthorNames(item.authors));
  ...
  return (
    ...
    <div className='dashboard-container'>
        <center><h1>View the Completed Reviews here!</h1></center>
  ...
  {completedSubmissions.map((submission, index) => (
    <tr key={index} style={{ background: "transparent" }}>
      ...
      <td style={cellStyle}><center><button className='nav-link login' onClick={() => {
            window.open(`http://${process.env.REACT_APP_HOST}/Submission-paper?submission_id=${submission.submission_id}&route_source=editor-review-completed-submissions`, '_self');
...
export default EditorCompletedReviewsDashboard
